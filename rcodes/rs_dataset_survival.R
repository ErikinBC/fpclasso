#############################################################################################
############# ----------- SCRIPT TO GENERATE SURVIVAL DATASETS --------------- ##############
#############################################################################################
...
# dir.dataset="/home/erik/Documents/projects/FPC_Lasso/current/datasets" #'C:/Users/erikinwest/Documents/Research/ices/FPC_Lasso/'
# dir.output= "/home/erik/Documents/projects/FPC_Lasso/current/output/data"#'C:/Users/erikinwest/Documents/Research/ices/FPC_Lasso/output/data'
...
pckgs <- c('data.table','survival', 'MASS', 'data.table', 'forcats','stringr',
           ...
           'pec','Ecdat','popEpi','relsurv', #RcmdrPlugin.survival
...
# # Check that there are no (almost) perfectly collinear columns
# cc.max <- rep(NA, length(lst.surv))
# names(cc.max) <- names(lst.surv)
# for (ii in which(!str_detect(names(lst.surv),'breast\\_')) ) {
#   print(sprintf('Dataset: %i, (%s)',ii,names(lst.surv)[ii]))
#   tmp.cor <- cor(lst.surv[[ii]]$X)
#   cc.max[ii] <- max(abs(tmp.cor[lower.tri(tmp.cor)]))
# }
# cc.max %>% summary

# load(file=file.path(dir.output,'surv_datasets.RData'))
# # Loop through and write
# dir.output <- file.path(dir.base, 'output')
# for (nn in names(lst.surv)) {
#   print(nn)
#   tmp.So <- data.table(as.matrix(lst.surv[[nn]]$So))
#   tmp.X <- data.table(as.matrix(lst.surv[[nn]]$X))
#   # Write
#   fwrite(x=tmp.So, file=file.path(dir.output,str_c('So_',nn,'.csv')))
#   fwrite(x=tmp.X, file=file.path(dir.output,str_c('X_',nn,'.csv')))
# }
